...
class BankAccount:
    ...
    def deposit(self, amount_to_add):
        if amount_to_add <= 0:
            print(
                f"Cannot add value to balance: {amount_to_add}. Deposit amount must be positive\n"
            )
            return
    ...
    def withdraw(self, amount_to_withdraw):
        ...
        if amount_to_withdraw <= 0:
            print(
                f"Cannot withdraw value: {amount_to_withdraw}. Amount to withdraw must be positive.\n"
            )
            return
